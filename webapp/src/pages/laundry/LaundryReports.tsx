...
export default function LaundryReports() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 space-y-6 duration-500">
      ...
      <div className="rounded-[22px] bg-[#123039] p-5 text-[#edf3ec] md:p-6"><p className="text-xs uppercase tracking-[.16em] text-[#b8d3c8]">Customer & balance snapshot</p><div className="mt-3 flex flex-wrap gap-x-10 gap-y-3"><span><strong className="font-serif text-2xl text-[#f1ca75]">{data.summary.orders}</strong><span className="ml-2 text-sm">orders in range</span></span><span><strong className="font-serif text-2xl text-[#f1ca75]">{data.summary.customers}</strong><span className="ml-2 text-sm">customers served</span></span><span><strong className="font-serif text-2xl text-[#f1ca75]">{formatINR(data.summary.outstanding)}</strong><span className="ml-2 text-sm">outstanding</span></span></div></div><section className="rounded-[22px] border border-[#263f44]/10 bg-white p-5 shadow-[0_8px_28px_rgba(37,48,43,.04)]"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.15em] text-[#4d8982]">Report centre</p><h2 className="mt-1 font-serif text-xl text-[#17353c]">15 verified report views</h2></div><Link to="/laundry/reports/invoice" className="text-xs font-bold text-[#39786f]">Open Invoice Report →</Link></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{[['invoice','Invoice Report'],['collection','Collection Report'],['order','Order Report'],['consolidated-invoices','Consolidated Invoices'],['customer','Customer Report'],['customer-package','Customer Package Report'],['customer-list','Customer List'],['growth','Growth Report'],['discount','Discount Report'],['expense','Expense Report'],['balance','Balance Report'],['pickup','Pickup Overview'],['rider-delivery','Rider Delivery'],['rider-collection','Rider Collection'],['warehouse-user-work','Warehouse User Work Report']].map(([id,label]) => <Link key={id} to={`/laundry/reports/${id}`} className="rounded-xl border border-[#263f44]/10 bg-[#fbfbf8] px-3 py-2.5 text-sm font-semibold text-[#315d57] transition hover:border-[#79a59b] hover:bg-[#f1f7f3]">{label}</Link>)}</div></section>
    </div>
  )
}
