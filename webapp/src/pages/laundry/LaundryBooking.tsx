...
import { Check, ChevronDown, CircleDollarSign, ImagePlus, Loader2, Minus, PackagePlus, Pause, PlayCircle, Plus, Printer, RotateCcw, Search, Tag, UserPlus, X } from 'lucide-react'
...
type TagData = { tagNumber: string; orderNumber: string; customer: string; garment: string; service: string; sequence: number; total: number; orderDate: string; expectedDeliveryDate: string }
type BookingResult = { receipt: Receipt; tags: TagData[] }
type BookingDraft = { cart: Record<string, CartLine>; customer: Customer | null; newCustomerName: string; newCustomerPhone: string; deliveryAddress: string; serviceZone: string; deliveryMode: 'Pickup Order' | 'Home Delivery' | 'Express Delivery'; expectedDeliveryDate: string; charges: number; discounts: number; taxRate: number; chargeRuleIds: string[]; discountRuleIds: string[]; taxRuleId: string; notes: string }
...
export default function LaundryBooking() {
  ...
  useEffect(() => {
    ...
    const draft: BookingDraft = { cart, customer, newCustomerName, newCustomerPhone, deliveryAddress, serviceZone, deliveryMode, expectedDeliveryDate, charges, discounts, taxRate, chargeRuleIds, discountRuleIds, taxRuleId, notes }
    try { window.localStorage.setItem(DRAFT_KEY, JSON.stringify(draft)) } catch { /* local storage is best effort */ }
  }, [draftRestored, cart, customer, newCustomerName, newCustomerPhone, deliveryAddress, serviceZone, deliveryMode, expectedDeliveryDate, charges, discounts, taxRate, chargeRuleIds, discountRuleIds, taxRuleId, notes])
  useEffect(() => {
  ...
  const booking = useMutation({
    mutationFn: () => { const cashRegister = paymentMode === 'Cash' ? window.prompt('Cash register (leave blank for the only open register):', '')?.trim() || undefined : undefined; return apiPostOffline<BookingResult>('/laundry/orders', {
      ...
      items, expectedDeliveryDate, fulfillmentMode: deliveryMode, serviceZone, cashRegister, paymentMode, paymentReference, charges, discounts, taxRate, chargeRuleIds, discountRuleIds, taxRuleId, notes, photoPaths: photoPath,
    }, 'laundry_order') },
    onSuccess: (result) => {
      setReceipt(result); setCart({}); setCustomer(null); setCustomerSearch(''); setNewCustomerName(''); setNewCustomerPhone(''); setDeliveryAddress(''); setServiceZone(''); setPhotoPath(''); setPhotoError(''); setPaymentReference(''); setPaymentMode('Pay Later'); setNotes(''); setChargeRuleIds([]); setDiscountRuleIds([]); setTaxRuleId('')
  ...
  function currentDraft(): BookingDraft {
    return { cart, customer, newCustomerName, newCustomerPhone, deliveryAddress, serviceZone, deliveryMode, expectedDeliveryDate, charges, discounts, taxRate, chargeRuleIds, discountRuleIds, taxRuleId, notes }
  }
  function clearCurrentDraft() {
    setCart({}); setCustomer(null); setCustomerSearch(''); setNewCustomerName(''); setNewCustomerPhone(''); setDeliveryAddress(''); setServiceZone(''); setPhotoPath(''); setPhotoError(''); setPaymentReference(''); setPaymentMode('Pay Later'); setNotes(''); setChargeRuleIds([]); setDiscountRuleIds([]); setTaxRuleId('')
  ...
  return <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
    ...
    <div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)_340px]">
      ...
      <section className="h-fit rounded-[22px] border border-[#263f44]/10 bg-[#fffdf8] p-4 shadow-[0_8px_28px_rgba(37,48,43,.05)] xl:sticky xl:top-24"><div className="flex items-center justify-between"><div><p className="font-serif text-xl text-[#17353c]">Selected</p><p className="text-xs text-[#74848a]">{items.length} garment line{items.length === 1 ? '' : 's'}</p></div><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#e6bc65]/35 text-[#72511d]"><Tag className="h-4 w-4" /></span></div><div className="mt-4 max-h-60 space-y-2 overflow-y-auto pr-1">{quoteQuery.data?.items.map((item) => <div key={`${item.garmentName}:${item.serviceName}`} className="rounded-xl bg-white p-3"><div className="flex justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-semibold">{item.garmentName}</p><p className="text-xs text-[#718087]">{item.serviceName} · {item.qty} {item.unit.toLowerCase()}</p></div><p className="text-sm font-bold tabular-nums">{formatINR(item.amount)}</p></div></div>)}{items.length === 0 && <div className="rounded-2xl border border-dashed border-[#aabbb4] px-4 py-10 text-center text-sm text-[#718087]">Choose garments from the catalogue to start this order.</div>}</div>
        ...
        <div className="mt-4"><p className="mb-2 text-[10px] font-bold uppercase tracking-[.15em] text-[#648077]">Payment</p><div className="grid grid-cols-2 gap-1.5">{(['Pay Later', 'Cash', 'UPI', 'Card', 'Bank'] as const).map((mode) => <button key={mode} onClick={() => setPaymentMode(mode)} className={cn('rounded-xl px-2 py-2 text-xs font-bold transition', paymentMode === mode ? 'bg-[#e6bc65] text-[#17363e]' : 'bg-white text-[#617178] ring-1 ring-inset ring-[#263f44]/10 hover:bg-[#f2f4ee]')}>{mode}</button>)}</div>{paymentMode !== 'Pay Later' && <input value={paymentReference} onChange={(event) => setPaymentReference(event.target.value)} placeholder="Payment reference / receipt no. (optional)" className="mt-2 h-9 w-full rounded-lg border border-[#263f44]/15 bg-white px-2.5 text-xs outline-none focus:border-[#438b82]" />}</div>
...
function ReceiptDialog({ result, onClose }: { result: BookingResult; onClose: () => void }) { const receipt = result.receipt; return <div className="fixed inset-0 z-50 grid place-items-center bg-[#102b33]/55 p-4 backdrop-blur-sm"><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-[24px] bg-[#fffdf8] p-5 shadow-2xl"><div className="flex justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#4d8982]">Order booked</p><h2 className="mt-1 font-serif text-2xl text-[#17353c]">{receipt.orderNumber}</h2></div><button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg hover:bg-[#f0eee9]" aria-label="Close receipt"><X className="h-4 w-4" /></button></div><div className="mt-5 rounded-2xl border border-dashed border-[#8daaa0] bg-white p-4"><div className="flex justify-between text-sm"><span>{receipt.customer.name}</span><span>{receipt.customer.phone}</span></div><p className="mt-1 text-xs text-[#718087]">Invoice {receipt.invoiceNumber} · Delivery {receipt.expectedDeliveryDate}</p><div className="mt-4 space-y-2 border-y border-[#263f44]/10 py-3">{receipt.items.map((item) => <div key={`${item.garmentName}:${item.serviceName}`} className="flex justify-between text-sm"><span>{item.garmentName} × {item.qty}</span><span>{formatINR(item.amount)}</span></div>)}</div><div className="mt-3 flex justify-between font-serif text-xl"><span>Total</span><span>{formatINR(receipt.grandTotal)}</span></div><p className="mt-2 text-xs text-[#718087]">{receipt.paymentStatus} · {receipt.paymentMode}</p></div><div className="mt-5"><p className="text-xs font-bold uppercase tracking-[.15em] text-[#648077]">Garment tags ({result.tags.length})</p><div className="mt-2 grid grid-cols-2 gap-2">{result.tags.map((tag) => <div key={tag.tagNumber} className="rounded-xl border border-[#263f44]/10 bg-white p-2.5 text-xs"><p className="font-bold text-[#225861]">{tag.tagNumber} <span className="font-normal text-[#718087]">({tag.sequence} / {tag.total})</span></p><p className="mt-1 font-medium">{tag.garment}</p><p className="text-[#718087]">{tag.service} · Due {tag.expectedDeliveryDate}</p><p className="text-[10px] text-[#91a09f]">Order date {tag.orderDate}</p></div>)}</div></div><button onClick={() => window.print()} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#123039] py-3 text-sm font-bold text-white"><Printer className="h-4 w-4" /> Print receipt & tags</button></div></div> }
