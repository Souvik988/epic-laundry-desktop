  ...
  return <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <section className="h-fit rounded-[22px] border border-[#263f44]/10 bg-[#fffdf8] p-3 shadow-[0_8px_28px_rgba(37,48,43,.05)] xl:sticky xl:top-24 xl:max-h-[calc(100vh-7rem)] xl:overflow-y-auto">
        ...
        <div className="mt-3 rounded-xl bg-[#123039] p-3 text-[#edf3ec]"><div className="flex justify-between text-[11px] text-[#c4d7d0]"><span>Sub total</span><span>{formatINR(quoteQuery.data?.subtotal || 0)}</span></div><div className="mt-1 flex justify-between text-[11px] text-[#c4d7d0]"><span>Adjustment</span><span>{formatINR((quoteQuery.data?.charges || 0) - (quoteQuery.data?.discounts || 0) + (quoteQuery.data?.taxAmount || 0))}</span></div><div className="mt-2 flex items-end justify-between border-t border-white/15 pt-2"><span className="font-serif text-base">Grand total</span><span className="font-serif text-xl tabular-nums text-[#f1ca75]">{formatINR(quoteQuery.data?.grandTotal || 0)}</span></div></div>
