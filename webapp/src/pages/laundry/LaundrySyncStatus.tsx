...
type SyncStatus = {
  ...
  inbox: { received: number; held: number; failed: number }
  onlineOrders: number
}
...
export default function LaundrySyncStatus() {
  ...
  const connected = data.configured && data.device?.status === 'Registered'
  return <div className="animate-in fade-in slide-in-from-bottom-2 space-y-6 duration-500">
    ...
    <section className={`rounded-[22px] border p-5 md:p-6 ${connected && !issueCount ? 'border-[#39786f]/20 bg-[#f3faf6]' : connected ? 'border-[#d89b4b]/30 bg-[#fff8e8]' : 'border-[#263f44]/10 bg-white'}`}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-3"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${connected && !issueCount ? 'bg-[#dcefe5] text-[#2e6a60]' : connected ? 'bg-[#ffedc8] text-[#9a6518]' : 'bg-[#eef2f0] text-[#6c7e80]'}`}>{connected ? (issueCount ? <ShieldAlert className="h-5 w-5" /> : <CheckCircle2 className="h-5 w-5" />) : <WifiOff className="h-5 w-5" />}</span><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#4d8982]">Connection state</p><h2 className="mt-1 font-serif text-2xl text-[#17353c]">{connected ? (issueCount ? 'Connected · needs attention' : 'Connected · healthy') : 'Local standalone · marketplace not configured'}</h2><p className="mt-1 text-sm text-[#617178]">{connected ? 'This device is registered locally. A cloud adapter and signed control-plane contract are still required before production marketplace traffic is enabled.' : 'Local booking, production, cash, printing, and reporting continue without marketplace dependency.'}</p></div></div><span className={`inline-flex w-fit items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold ${connected ? statusTone(data.device?.status || '') : 'bg-[#eef2f0] text-[#617178]'}`}><Cloud className="h-3.5 w-3.5" />{connected ? data.device?.status : 'Not configured'}</span></div>
    </section>
