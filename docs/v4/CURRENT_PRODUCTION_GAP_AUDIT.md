...
Audit date: 2026-09-02. Current local baseline inspected: `0b5f4d7` on local `main`. Remote `origin/main` remains `72767269e83d905c7f484d8ce08ae7d32a27b1bc`; no remote write has been performed.
...
| Catalogue imagery | BUG → partially fixed | Booking previously inferred visuals from free-text names and defaulted unknown garments to a shirt. V4 now uses explicit visual keys, corrected eleven wrong active mappings, generated a complete contact sheet, and ships every active runtime image as a 256px WebP derivative (zero asset-budget warnings). Eight closely related generic/shared mappings still require visual-owner review. |
