...
Audit date: 2026-09-03. Current local baseline inspected: `a6ad5f3` on local `main`. Remote `origin/main` remains `72767269e83d905c7f484d8ce08ae7d32a27b1bc`; no remote write has been performed.
...
| Tax / invoice compliance | PARTIAL / NEEDS_LEGAL_VALIDATION | Fixed-scale line-level tax, effective-dated policy, supplier profile/readiness, immutable canonical snapshot and no-fake-QR print route exist. Configured-store laundry bookings now create and link the canonical snapshot, and the legacy tax print route bridges through it with fail-closed classification/profile checks. Cancellation credit-note lifecycle, remaining ERP document paths, and legal/provider evidence for production GST/e-invoice claims remain incomplete. Marketplace materialization now fails explicitly when the supplier profile is incomplete. |
