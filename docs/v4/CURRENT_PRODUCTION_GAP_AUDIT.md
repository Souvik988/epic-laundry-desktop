...
Audit date: 2026-09-03. Current local baseline inspected: `22971f4` on local `main`. Remote `origin/main` remains `72767269e83d905c7f484d8ce08ae7d32a27b1bc`; no remote write has been performed.
...
| Online Orders operator surface | EPIC_EXTENSION / PARTIAL | Runtime-inspected queue cockpit now exposes source, request, payment, sync, acceptance/rejection, physical intake entry and materialization controls. Queue visibility is available to `orders.read`; mutations remain `orders.edit`-protected with a clear read-only state. Main store-order search now uses bounded SQL-backed pages with deterministic ordering and a maintained FTS5 projection. A 100k-customer/500k-order fixture measured 502 ms customer-name search; deep offset paging measured 1.24 seconds and remains an optimization target. Customer timeline, pickup scheduling and richer work-card integration remain. |
