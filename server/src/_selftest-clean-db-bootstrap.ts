...
try {
  ...
  assert.ok(store.migrationStatus().length >= 25, 'first Store instantiation applies schema migrations');

  store.close();
  closeStore = undefined;
  const restarted = new Store(databaseFile, { skipLegacyImport: true });
  closeStore = () => restarted.close();
  assert.ok(restarted.migrationStatus().length >= 25, 'database restarts with the applied schema');
