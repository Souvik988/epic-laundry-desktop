...
try {
  ...
  const { approveTaxPolicyRule, createTaxPolicyRule, listTaxPolicyRules, resolveTaxPolicyRule, saveSupplierTaxProfile, supplierStateCodeForTenant, supplierTaxProfile, taxReadiness } = await import('./modules/gst/tax-policy.js');
  close = () => store.close();
  ...
  process.env.EPIC_WORKSPACE_MODE = 'production';
  assert.throws(() => store.withStoreScope('TAX-POLICY-MISSING-PROFILE', 'STORE-TAX', () => supplierStateCodeForTenant('TAX-POLICY-MISSING-PROFILE')), /TAX_PROFILE_INCOMPLETE/, 'production supplier state resolution fails closed without a persisted tax profile');
