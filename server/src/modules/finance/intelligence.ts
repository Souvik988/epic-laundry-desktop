...
import { statutoryDashboard } from './statutory.js';
import { actualVsTarget } from './planning.js';
import { packageLiability as packageLiabilitySummary } from '../laundry/packages.js';
...
const storedPaise = (value: unknown, label: string) => { const result = typeof value === 'number' ? value : Number(value ?? 0); if (!Number.isSafeInteger(result) || result < 0) throw new Error(`${label} must be a non-negative integer paise amount`); return result; };
...
function runRateForecast(current: ReturnType<typeof aggregate>) {
  const observedDays = current.trend.length;
  const observed = current.trend.reduce((sum, point) => ({
    netRevenuePaise: sum.netRevenuePaise + point.netRevenuePaise,
    expensesPaise: sum.expensesPaise + point.expensesPaise,
    cashFlowPaise: sum.cashFlowPaise + point.cashInPaise - point.cashOutPaise,
  }), { netRevenuePaise: 0, expensesPaise: 0, cashFlowPaise: 0 });
  const hasEvidence = observedDays > 0 && (observed.netRevenuePaise > 0 || observed.expensesPaise > 0 || observed.cashFlowPaise !== 0);
  if (!hasEvidence) return { state: 'NOT_ENOUGH_DATA' as const, method: 'DAILY_RUN_RATE', horizonDays: 0, points: [] };
  const horizonDays = Math.min(14, Math.max(7, observedDays));
  const daily = {
    netRevenuePaise: Math.round(observed.netRevenuePaise / observedDays),
    expensesPaise: Math.round(observed.expensesPaise / observedDays),
    cashFlowPaise: Math.round(observed.cashFlowPaise / observedDays),
  };
  return {
    state: 'RUN_RATE' as const,
    method: 'DAILY_RUN_RATE',
    horizonDays,
    daily: { ...daily, netRevenue: rupees(daily.netRevenuePaise), expenses: rupees(daily.expensesPaise), cashFlow: rupees(daily.cashFlowPaise) },
    points: Array.from({ length: horizonDays }, (_, index) => {
      const date = addDays(current.range.to, index + 1);
      return { date, netRevenuePaise: daily.netRevenuePaise, netRevenue: rupees(daily.netRevenuePaise), expensesPaise: daily.expensesPaise, expenses: rupees(daily.expensesPaise), operatingCashFlowPaise: daily.cashFlowPaise, operatingCashFlow: rupees(daily.cashFlowPaise) };
    }),
    note: 'Deterministic daily run-rate from the selected period; this is not an AI forecast and contains no target assumption.',
  };
}

function healthState(value: 'Healthy' | 'Watch' | 'Critical', basis: string) { return { state: value, basis }; }

function financialHealth(current: ReturnType<typeof aggregate>, readiness: ReturnType<typeof financePolicyReadiness>) {
  const { kpis, classification, receivables, settlement } = current;
  const marginState = kpis.ebitdaPaise === null ? healthState('Watch', 'EBITDA is withheld until every expense has a management classification.') : kpis.ebitdaPaise < 0 ? healthState('Critical', 'Classified operating costs exceed net revenue in the selected period.') : healthState('Healthy', 'Classified EBITDA is non-negative in the selected period.');
  const cashState = kpis.operatingCashFlowPaise < 0 ? healthState('Critical', 'Posted cash outflows exceed posted cash inflows.') : kpis.operatingCashFlowPaise === 0 ? healthState('Watch', 'No positive operating cash movement is evidenced.') : healthState('Healthy', 'Posted operating cash flow is positive.');
  const receivableState = receivables.totalPaise === 0 ? healthState('Healthy', 'No outstanding invoice balance is evidenced.') : receivables.overduePaise > receivables.totalPaise / 2 ? healthState('Critical', 'More than half of outstanding receivables are older than 15 days.') : healthState('Watch', 'Outstanding receivables require collection follow-up.');
  const settlementDuePaise = settlement.vendorSettlementPaise?.paise || 0;
  const settlementState = settlementDuePaise > 0 ? healthState('Watch', 'Vendor settlement liability is present and requires reconciliation before payout.') : healthState('Healthy', 'No open vendor settlement liability is evidenced in this period.');
  const taxState = readiness.checks.statutoryBaseline && readiness.checks.entityConfiguration ? healthState('Healthy', 'Statutory baseline and entity profile are configured.') : healthState('Watch', 'Finance setup is incomplete; compliance-dependent actions remain gated.');
  return [
    { key: 'margin', label: 'Margin health', ...marginState },
    { key: 'cash', label: 'Cash health', ...cashState },
    { key: 'receivables', label: 'Receivable health', ...receivableState },
    { key: 'expenseControl', label: 'Expense control', ...(classification.complete ? healthState('Healthy', 'All period expenses have a management classification.') : healthState('Watch', `${classification.unclassifiedExpenses} period expense record(s) still need classification.`)) },
    { key: 'settlement', label: 'Settlement health', ...settlementState },
    { key: 'tax', label: 'Tax readiness', ...taxState },
  ];
}

function aggregate(tenant: string, range: Range) {
  const trend = dailyPoints(range); const issues: string[] = [];
  let grossRevenuePaise = 0; let discountsPaise = 0; let taxPaise = 0; let activeOrders = 0; let activeItemUnits = 0;
  const service = new Map<string, number>(); const serviceOrders = new Map<string, Set<string>>(); const channel = new Map<string, number>(); const discountByChannel = new Map<string, number>();
  ...
  for (const order of store.rowsOf(tenant, 'laundry_order')) {
    ...
    const source = String(order.data.source || 'Counter'); channel.set(source, (channel.get(source) || 0) + Math.max(0, serviceRevenue - discountPaise)); discountByChannel.set(source, (discountByChannel.get(source) || 0) + discountPaise);
    for (const item of Array.isArray(order.data.items) ? order.data.items as Array<Record<string, unknown>> : []) {
      const name = String(item.serviceName || 'Unmapped service'); const amount = safePaise(item.amount, `order ${order.id} item`); service.set(name, (service.get(name) || 0) + amount); const orderSet = serviceOrders.get(name) || new Set<string>(); orderSet.add(order.id); serviceOrders.set(name, orderSet); const quantity = Number(item.qty ?? item.quantity ?? 0); if (Number.isFinite(quantity) && quantity >= 0) activeItemUnits += quantity;
    }
  }
  ...
  const taxTrendMap = new Map<string, { date: string; taxablePaise: number; cgstPaise: number; sgstPaise: number; igstPaise: number }>(); for (const date of trend.keys()) taxTrendMap.set(date, { date, taxablePaise: 0, cgstPaise: 0, sgstPaise: 0, igstPaise: 0 }); for (const snapshot of store.rowsOf(tenant, 'canonical_invoice_snapshot').filter((row) => row.data.documentType === 'TaxInvoice')) { const issuedDate = dateKey(String(snapshot.data.issuedAt || snapshot.created_at)); if (!inRange(issuedDate, range)) continue; const tax = snapshot.data.tax as any; const point = taxTrendMap.get(issuedDate); if (!point) continue; point.taxablePaise += storedPaise(tax?.totals?.taxablePaise || 0, `tax snapshot ${snapshot.id}`); point.cgstPaise += storedPaise(tax?.totals?.cgstPaise || 0, `tax snapshot ${snapshot.id}`); point.sgstPaise += storedPaise(tax?.totals?.sgstPaise || 0, `tax snapshot ${snapshot.id}`); point.igstPaise += storedPaise(tax?.totals?.igstPaise || 0, `tax snapshot ${snapshot.id}`); }
  const qualityByCategory = new Map<string, number>(); const qualityByService = new Map<string, number>(); let qualityClaims = 0; for (const claim of store.rowsOf(tenant, 'laundry_quality_claim')) { const openedDate = dateKey(String(claim.data.opened_at || claim.created_at)); if (!inRange(openedDate, range)) continue; qualityClaims += 1; const category = String(claim.data.category || 'Other'); qualityByCategory.set(category, (qualityByCategory.get(category) || 0) + 1); const unit = store.getGarmentUnit(tenant, String(claim.data.garment_unit || '')); const serviceRow = unit ? store.getRow(tenant, unit.serviceId) : undefined; const serviceName = String(serviceRow?.data.name || 'Unmapped service'); qualityByService.set(serviceName, (qualityByService.get(serviceName) || 0) + 1); }
  const returnByReason = new Map<string, { count: number; requestedPaise: number }>(); let returnCases = 0; let returnRequestedPaise = 0; for (const item of store.rowsOf(tenant, 'laundry_return_case')) { const createdDate = dateKey(item.created_at); if (!inRange(createdDate, range) || item.status === 'Cancelled') continue; const amountPaise = safePaise(item.data.amount || 0, `return ${item.id}`); const reason = String(item.data.reason || 'Other'); const bucket = returnByReason.get(reason) || { count: 0, requestedPaise: 0 }; bucket.count += 1; bucket.requestedPaise += amountPaise; returnByReason.set(reason, bucket); returnCases += 1; returnRequestedPaise += amountPaise; }
  const walletEntries = store.listWalletEntries(tenant).filter((entry) => entry.entryDate <= range.to); const walletCreditsPaise = walletEntries.filter((entry) => entry.entryType !== 'Debit').reduce((sum, entry) => sum + entry.amountPaise, 0); const walletDebitsPaise = walletEntries.filter((entry) => entry.entryType === 'Debit').reduce((sum, entry) => sum + entry.amountPaise, 0); const walletBalancePaise = walletCreditsPaise - walletDebitsPaise; if (walletBalancePaise < 0) issues.push('Wallet liability is negative after posted credits and debits.');
  let packageSnapshot = { contractPaise: 0, collectedPaise: 0, outstandingPaise: 0, remainingUnits: 0, packageCount: 0, issueCount: 0 }; try { const report = packageLiabilitySummary(tenant); const packages = report.packages.filter((item: any) => String(item.purchasedDate) <= range.to); packageSnapshot = { contractPaise: packages.reduce((sum: number, item: any) => sum + Number(item.contractPaise || 0), 0), collectedPaise: packages.reduce((sum: number, item: any) => sum + Number(item.collectedPaise || 0), 0), outstandingPaise: packages.reduce((sum: number, item: any) => sum + Number(item.outstandingPaise || 0), 0), remainingUnits: Math.round(packages.reduce((sum: number, item: any) => sum + Number(item.remainingUnits || 0), 0) * 100) / 100, packageCount: packages.length, issueCount: Number(report.checks?.issueCount || 0) }; } catch (error: any) { issues.push(`Package liability: ${error.message || 'could not be calculated'}`); }
  ...
  return { range, trend: [...trend.values()].map(presentPoint), kpis: { grossRevenuePaise, grossRevenue: rupees(grossRevenuePaise), netRevenuePaise, netRevenue: rupees(netRevenuePaise), collectedPaise, collected: rupees(collectedPaise), expensesPaise, expenses: rupees(expensesPaise), cashInPaise, cashIn: rupees(cashInPaise), cashOutPaise, cashOut: rupees(cashOutPaise), operatingCashFlowPaise, operatingCashFlow: rupees(operatingCashFlowPaise), outstandingPaise, outstanding: rupees(outstandingPaise), refundsPaise, refunds: rupees(refundsPaise), directCostPaise, directCost: rupees(directCostPaise), grossProfitPaise, grossProfit: grossProfitPaise === null ? null : rupees(grossProfitPaise), contributionPaise, contribution: contributionPaise === null ? null : rupees(contributionPaise), ebitdaPaise, ebitda: ebitdaPaise === null ? null : rupees(ebitdaPaise), orderCount: activeOrders, itemUnits: activeItemUnits, averageOrderValuePaise: activeOrders ? Math.round(netRevenuePaise / activeOrders) : null, grossMarginPercent: grossProfitPaise === null ? null : percent(grossProfitPaise, netRevenuePaise), contributionMarginPercent: contributionPaise === null ? null : percent(contributionPaise, netRevenuePaise), ebitdaMarginPercent: ebitdaPaise === null ? null : percent(ebitdaPaise, netRevenuePaise) }, unitEconomics: { averageItemsPerOrder: activeOrders ? Math.round(activeItemUnits / activeOrders * 100) / 100 : null, revenuePerOrderPaise: activeOrders ? Math.round(netRevenuePaise / activeOrders) : null, revenuePerOrder: activeOrders ? rupees(Math.round(netRevenuePaise / activeOrders)) : null, grossProfitPerOrderPaise: grossProfitPaise === null || !activeOrders ? null : Math.round(grossProfitPaise / activeOrders), grossProfitPerOrder: grossProfitPaise === null || !activeOrders ? null : rupees(Math.round(grossProfitPaise / activeOrders)), contributionPerOrderPaise: contributionPaise === null || !activeOrders ? null : Math.round(contributionPaise / activeOrders), contributionPerOrder: contributionPaise === null || !activeOrders ? null : rupees(Math.round(contributionPaise / activeOrders)) }, prepaid: { walletCreditsPaise: walletCreditsPaise, walletCredits: rupees(walletCreditsPaise), walletDebitsPaise: walletDebitsPaise, walletDebits: rupees(walletDebitsPaise), walletBalancePaise: walletBalancePaise, walletBalance: rupees(Math.max(0, walletBalancePaise)), packageContractPaise: packageSnapshot.contractPaise, packageContract: rupees(packageSnapshot.contractPaise), packageCollectedPaise: packageSnapshot.collectedPaise, packageCollected: rupees(packageSnapshot.collectedPaise), packageOutstandingPaise: packageSnapshot.outstandingPaise, packageOutstanding: rupees(packageSnapshot.outstandingPaise), packageRemainingUnits: packageSnapshot.remainingUnits, packageCount: packageSnapshot.packageCount, issueCount: packageSnapshot.issueCount, revenueRecognitionState: 'MANAGEMENT_POLICY_REQUIRED' as const }, classification: { policy: 'MANAGEMENT-2026.1', state: classificationComplete ? 'MANAGEMENT_POLICY' : 'NEEDS_CLASSIFICATION', unclassifiedExpenses, complete: classificationComplete }, composition: { expense: [...expenseBuckets.values()].sort((a, b) => b.amountPaise - a.amountPaise).map((item) => ({ ...item, amount: rupees(item.amountPaise) })), services: [...service].map(([name, amountPaise]) => ({ name, amountPaise, orderCount: serviceOrders.get(name)?.size || 0, amount: rupees(amountPaise), averageOrderRevenue: serviceOrders.get(name)?.size ? rupees(Math.round(amountPaise / serviceOrders.get(name)!.size)) : null, qualityClaims: qualityByService.get(name) || 0, contributionState: classificationComplete ? 'COST_ALLOCATION_REQUIRED' : 'NOT_READY' })).sort((a, b) => b.amountPaise - a.amountPaise), channels: [...channel].map(([name, amountPaise]) => ({ name, amountPaise, amount: rupees(amountPaise), discountsPaise: discountByChannel.get(name) || 0, discounts: rupees(discountByChannel.get(name) || 0) })).sort((a, b) => b.amountPaise - a.amountPaise), payments: [...paymentMode].map(([name, amountPaise]) => ({ name, amountPaise, amount: rupees(amountPaise) })).sort((a, b) => b.amountPaise - a.amountPaise), discounts: [...discountByChannel].map(([name, amountPaise]) => ({ name, amountPaise, amount: rupees(amountPaise) })).sort((a, b) => b.amountPaise - a.amountPaise) }, receivables: { totalPaise: outstandingPaise, total: rupees(outstandingPaise), overduePaise, overdue: rupees(overduePaise), collectionRatePercent: percent(collectedPaise, grossRevenuePaise - discountsPaise), aging: [...receivableBuckets].map(([label, amountPaise]) => ({ label, amountPaise, amount: rupees(amountPaise) })) }, settlement: Object.fromEntries(Object.entries(settlement).map(([key, value]) => [key, { paise: value, amount: rupees(value) }])), tax: { ...canonicalTax, taxable: rupees(canonicalTax.taxablePaise), cgst: rupees(canonicalTax.cgstPaise), sgst: rupees(canonicalTax.sgstPaise), igst: rupees(canonicalTax.igstPaise), legacyOutputTaxPaise: taxPaise, legacyOutputTax: rupees(taxPaise), canonicalInvoiceCount: canonicalTax.count, trend: [...taxTrendMap.values()].map((point) => ({ ...point, taxable: rupees(point.taxablePaise), cgst: rupees(point.cgstPaise), sgst: rupees(point.sgstPaise), igst: rupees(point.igstPaise) })) }, quality: { claims: qualityClaims, byCategory: [...qualityByCategory].map(([name, count]) => ({ name, count })), actualRefundPaise: refundsPaise, actualRefund: rupees(refundsPaise), estimatedCostPaise: null, estimatedCost: null, costState: 'RECORDED_REFUNDS_ONLY' }, returns: { cases: returnCases, requestedPaise: returnRequestedPaise, requested: rupees(returnRequestedPaise), byReason: [...returnByReason].map(([name, value]) => ({ name, count: value.count, requestedPaise: value.requestedPaise, requested: rupees(value.requestedPaise) })) }, issues };
}
...
export function financeCommandCenter(tenant: string, input: { from?: string; to?: string } = {}) {
  ...
  return { generatedAt: new Date().toISOString(), currency: 'INR', range, previousRange: previous.range, current, previous: { kpis: previous.kpis }, comparisons: { netRevenue: comparison(currentKpis.netRevenuePaise, previous.kpis.netRevenuePaise), collected: comparison(currentKpis.collectedPaise, previous.kpis.collectedPaise), expenses: comparison(currentKpis.expensesPaise, previous.kpis.expensesPaise), operatingCashFlow: comparison(currentKpis.operatingCashFlowPaise, previous.kpis.operatingCashFlowPaise), ebitda: currentKpis.ebitdaPaise === null || previous.kpis.ebitdaPaise === null ? null : comparison(currentKpis.ebitdaPaise, previous.kpis.ebitdaPaise) }, forecast: runRateForecast(current), planning: actualVsTarget(tenant, range, current), health: financialHealth(current, readiness), compliance: { readiness: readiness.checks, missing: readiness.missing, outputGst: compliance.output_gst, inputGst: compliance.input_gst, netGstPayable: compliance.net_gst_payable, tdsPayable: compliance.tds_payable, tcsPayable: compliance.tcs_payable, policyState: readiness.checks.entityConfiguration ? 'CONFIGURATION_REVIEW' : 'ENTITY_CONFIGURATION_REQUIRED' }, statutory: statutoryDashboard(tenant, { from: range.from, to: range.to }), insights };
}
