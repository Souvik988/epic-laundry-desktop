...
function aggregate(tenant: string, range: Range) {
  ...
  for (const order of store.rowsOf(tenant, 'laundry_order')) {
    ...
    for (const item of Array.isArray(order.data.items) ? order.data.items as Array<Record<string, unknown>> : []) {
      const name = String(item.serviceName || 'Unmapped service'); const amount = safePaise(item.amount, `order ${order.id} item`); service.set(name, (service.get(name) || 0) + amount); const quantity = Number(item.qty ?? item.quantity ?? 0); if (Number.isFinite(quantity) && quantity >= 0) activeItemUnits += quantity;
    }
  }
