...
export function actOnMarketplaceOrder(tenant: string, actor: string, externalOrderId: string, input: { action: 'accept' | 'reject'; reason?: string }) {
  ...
  const now = new Date().toISOString(); const next: MarketplaceOrderProjectionRecord = { ...order, state: input.action === 'accept' ? 'Accepted' : 'Rejected', sourceVersion: order.sourceVersion + 1, notes: input.action === 'reject' ? `${order.notes}${order.notes ? '\n' : ''}Rejected: ${reason}` : order.notes, updatedAt: now, syncState: 'PendingOutbound' };
  const saved = store.saveMarketplaceOrderProjection(next);
  const event = queueMarketplaceEvent(tenant, actor, { aggregateType: 'marketplace_order', aggregateId: order.externalOrderId, aggregateVersion: saved.sourceVersion, eventType: input.action === 'accept' ? 'marketplace.order.accepted.v1' : 'marketplace.order.rejected.v1', payload: { externalOrderId: order.externalOrderId, localProjectionId: order.id, state: next.state, reason: reason || undefined, actedAt: now, vendorId: device.vendorId } });
  audit(tenant, actor, input.action === 'accept' ? 'marketplace:order-accepted' : 'marketplace:order-rejected', { entity: 'marketplace_order_projection', row_id: saved.id, after: { externalOrderId: order.externalOrderId, state: next.state, sourceVersion: saved.sourceVersion, reason: reason || undefined } });
...
export function marketplaceSyncStatus(tenant: string) {
  ...
  const conflicts = store.listSyncInbox(tenant).filter((event) => event.applyStatus === 'Held' && /VERSION|CONFLICT|MISMATCH/i.test(String(event.error || ''))).length;
