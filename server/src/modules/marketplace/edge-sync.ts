...
export function marketplaceSyncStatus(tenant: string) {
  ...
  return { version: SYNC_VERSION, configured: Boolean(device?.status === 'Registered'), device: device ? { id: device.id, vendorId: device.vendorId, storeId: device.storeId, station: device.station, status: device.status, lastSeenAt: device.lastSeenAt, rotationRequired: device.rotationRequired } : null, outbox: { pending: outbox.Pending, inFlight: outbox.InFlight, retry: outbox.Retry, acknowledged: outbox.Acknowledged, deadLetter: outbox.DeadLetter }, inbox: { received: inbox.Received, held: inbox.Held, failed: inbox.Failed }, onlineOrders: store.marketplaceOrderProjectionCount(tenant) };
}
