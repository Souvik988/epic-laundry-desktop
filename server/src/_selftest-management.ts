...
try {
  ...
  const tenant = 'MANAGEMENT'; const actor = 'owner'; const runDate = new Date().toISOString().slice(0, 10);
  ...
  const first = markLaundryAttendance(tenant, actor, { employee: employee.id, date: runDate, status: 'Present', shift: 'Day', workingHours: 8 });
  assert.equal(first.attendance.status, 'Submitted', 'Laundry attendance is submitted evidence, not a draft');
  assert.equal(laundryWorkforceDashboard(tenant, runDate).statusCounts.Present, 1, 'workforce dashboard derives current capacity from submitted attendance');
  assert.equal(markLaundryAttendance(tenant, actor, { employee: employee.id, date: runDate, status: 'Present' }).duplicate, true, 'same attendance retry is idempotent');
  assert.throws(() => markLaundryAttendance(tenant, actor, { employee: employee.id, date: runDate, status: 'Absent' }), /ALREADY_MARKED/, 'conflicting daily attendance cannot silently overwrite evidence');
  ...
  const booking = bookLaundryOrder(tenant, actor, { orderDate: runDate, customer: { name: 'Return Customer', phone: '9000000123' }, items: [{ garment: garment.id, service: service.id, qty: 1 }], expectedDeliveryDate: '2026-09-05', fulfillmentMode: 'Pickup Order', paymentMode: 'Pay Later' });
  createLaundryExpense(tenant, actor, { expenseName: 'Processing chemicals', expenseDate: runDate, amount: 10, financeCategory: 'PROCESSING', paymentMode: 'Bank' });
  store.insertRow({ id: 'tax-test-snapshot', entity: 'canonical_invoice_snapshot', tenant, status: 'Submitted', version: 1, created_by: actor, created_at: `${runDate}T10:00:00.000Z`, updated_at: `${runDate}T10:00:00.000Z`, data: { documentType: 'TaxInvoice', issuedAt: `${runDate}T10:00:00.000Z`, tax: { totals: { taxablePaise: 10_000, cgstPaise: 900, sgstPaise: 900, igstPaise: 0 } } } });
  const finance = financeCommandCenter(tenant, { from: runDate, to: runDate });
  ...
  // When a selected period has no posted orders, the unit-economics contract
  // correctly returns null rather than inventing a per-order value.
  assert.equal(finance.current.unitEconomics.revenuePerOrderPaise, finance.current.kpis.orderCount ? finance.current.kpis.netRevenuePaise : null, 'unit economics revenue per order is derived from canonical net revenue when orders exist');
  ...
  saveFinancePlanningTarget(tenant, actor, { periodStart: runDate, periodEnd: runDate, revenuePaise: finance.current.kpis.netRevenuePaise, collectionPaise: finance.current.kpis.collectedPaise, marginBps: 5000 });
  const plannedFinance = financeCommandCenter(tenant, { from: runDate, to: runDate });
