...
try {
  ...
  store.withStoreScope(tenant, 'STORE-ASSET', () => { store.insertRow({ ...base, id: 'g-exact', entity: 'laundry_garment', data: { name: 'Shirt', category: 'tops', visual_key: 'foldedShirt', photo: '/ui/app/garments/optimized/lndry-folded-shirt-v3.webp', active: true } }); store.insertRow({ ...base, id: 'g-shared', entity: 'laundry_garment', data: { name: 'Formal shirt', category: 'tops', visual_key: 'foldedShirt', active: true } }); store.insertRow({ ...base, id: 'g-bad', entity: 'laundry_garment', data: { name: 'Unknown', category: 'tops', active: true } }); });
  const report = store.withStoreScope(tenant, 'STORE-ASSET', () => auditGarmentAssets(tenant));
  assert.equal(report.total, 3); assert.equal(report.counts.exact_visual, 1); assert.equal(report.counts.intentional_shared_visual, 1); assert.equal(report.counts.missing, 1); assert.equal(report.ok, false); assert.equal(report.failures, 1);
